...
Instructor: Harold Stolper & Class: $if(classmeetings)$$classmeetings$$else$TBD$endif$\\
Pronouns: he/they & Recitation: $if(recitation)$$recitation$$else$TBD$endif$\\
E-mail: $if(email)$\texttt{$email$}$else$TBD$endif$ & TA: $if(ta)$$ta$$else$TBD$endif$\\
OH: $if(officehours)$$officehours$$else$TBD$endif$  &  TA OH: $if(taoh)$$taoh$$else$TBD$endif$\\
Course website: $if(website)$\texttt{$website$}$else$TBD$endif$ & (book via Courseworks Calendar)\\
	&  \\
	\hline
	\end{tabular*}
