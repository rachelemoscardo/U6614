...
xmluse  "$output/figureA2B4_fs_chrate.xls", clear  cells(A3:B34) first
drop in 1
...
foreach stat in pre post ddtest pltest wlstest {
	...
	local `stat' : display %03.2f round(r(mean)*1000)/100
}
...
graph set window fontface "Times New Roman"							/*FONT: make everything always show up as Times New Roman*/
	...
	twoway (scatter X3_1 lb ub exp, 
			...
			text(1.05 -1.5 "DD Estimate:"  "`dd' (s.e. = `ddse')", j(left) size(medsmall) box fcolor(white) margin(small))			
			graphregion(fcolor(white) color(white) icolor(white) margin(tiny)) )
			(pcarrowi 3.60 .02 2.8 -.75, lcolor(black) mcolor(black) lwidth(medthick) mlwidth(medthick)
			text(3.805 1.2 "Year Before Medicaid", j(left) size(medium)));
	#delimit cr;	
	
graph export "$output/figureA2B4_fs_chrate_firststage_kids.emf", replace
