...
#4c.
  ...
  #get separate data frames for predominantly Black/non-Black stations (nblack)
    stations_black <- stations %>% 
      filter(nblack == "Majority Black")
    
    stations_nblack <- stations %>% 
      filter(nblack == "Majority non-Black")
  
  #compare linear and quadratic specifications for stations_black
    ols_l_b <- lm(arrperswipe ~ povrt_all_2016, data = stations_black, weights = swipes2016)
    summary(ols_l_b) #get summary of the model
    summary(ols_l_b)$r.squared
    
    ols_q_b <- lm(arrperswipe ~ povrt_all_2016 + I(povrt_all_2016^2), data = stations_black, weights = swipes2016)
    summary(ols_q_b)
    summary(ols_q_b)$r.squared

  #compare linear and quadratic specifications for stations_nblack
    ols_l_nb <- lm(arrperswipe ~ povrt_all_2016, data = stations_nblack, weights = swipes2016)
    summary(ols_l_nb)
      
    ols_q_nb <- lm(arrperswipe ~ povrt_all_2016 + I(povrt_all_2016^2), data = stations_nblack, weights = swipes2016)
    summary(ols_q_nb)
