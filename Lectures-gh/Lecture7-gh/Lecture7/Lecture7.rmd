...
Note how salient major limitation of fixed effects models is in this example: any factors that within-countries over time cannot be accounted for with fixed effects. Said another way, you can't have country-month fixed effects, because that would attempt to control for the policy variable under consideration; it would introduce perfect multicollinearity between the treatment dummy variable and the corresponding country-year fixed effect, leading one of them to be dropped from the regression).

What's an example of a threat to internal validity that fixed effects can't account for this research question? If a particular country's decision to implement a national mask mandate is made in tandem with other effective public health measures like free testing programs, then it is impossible to distinguish between the relative contributions of these two policies using fixed effects alone. Maybe you can start to explicitly control for these other policies and confounding factors with the available data, but there may be major limitations to that approach.


***Self-assessment question:*** 

*Given the hypothetical panel data structure implied above, can you think of additional research design features and/or data that might help researchers to distinguish the effects of a mask-mandate from any national testing policies implemented at the same time?*
...
The `tidyr` package include a number of useful functions for restructuring data frames, and the `panelr` package is particularly useful for reshaping panel data.
...
Also note that you need to think carefully about the underlying error structure when estimating standard errors for inference on <span class="math inline">\(\hat\beta_1\)</span>. Examples of R functions for different standard error formulas for the LSDV and fixed effects models can be found below in Sections 5.1 and 5.2.
...
Note that the estimated slope effect of GDP per capita on life expectancy is negative! In other words, once we control for time-invariant country-specific characteristics and year-specific factors shared by all countries, we estimate a negative association between GDP per capita and life expectancy -- presumably contrary to our expectations!
...
lifeExp_r <- resid(lm(gap_long$lifeExp ~ factor(gap_long$country) + factor(gap_long$wave)))
gdpPercap_r <- resid(lm(gap_long$gdpPercap ~ factor(gap_long$country) + factor(gap_long$wave)))
