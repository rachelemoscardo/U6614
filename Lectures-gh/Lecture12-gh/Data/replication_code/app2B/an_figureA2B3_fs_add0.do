...
use stfips region year exp ymcaid crate0 bpc hpc pcinc popch white using "$data/vs_mcaid" if year==1950, clear
collapse (mean) bpc hpc pcinc ymcaid (rawsum) pop= popch [aw=popch], by(stfips region year)
ren pop popch
expand 5
drop year
sort stfips
by stfips: gen year = _n+1944
gen rmvp_ch = 0

append using "$data/mvp_mcaid"
drop if stfips==54|stfips==4
egen x = mean(crate0), by(stfips)
replace crate0 = x if year<1963
drop x
cap drop HC*
cap drop _Texp*
cap drop exp
...
gen exp = year - ymcaid
drop if inrange(exp,-27,-22) | inrange(exp,-20,-17)
recode exp (-20/-4 = -20) (7/1000 = 7)
char exp[omit] -1
xi i.exp, pref(_T)
...
cap drop _DD* 
cap drop D
egen D = cut(exp), at(-100,-21,-20,-3,-1,0,1,4,7,100)
char D[omit] -1
xi i.D, pref(_D)
...
local DX1 = "hc _Texp* HC_Texp_2 HC_Texp_6 HCPOST HC_Texp_13 [aw=ch_pop]"
local DX2 = "YMXY* pcinc hpc bpc HC_Texp_2 HC_Texp_6 HCPOST HC_Texp_13 [aw=ch_pop]"
local DX3 = "YMXY* RXY* pcinc hpc bpc HC_Texp_2 HC_Texp_6 HCPOST HC_Texp_13 [aw=ch_pop]"
local DX4 = "YMXY* RXY* pcinc hpc bpc HC_Texp_2 HC_Texp_6 HCPOST HC_Texp_13"
local DX5 = "SY_* YMXY* RXY* pcinc hpc bpc HC_Texp_2 HC_Texp_6 HCPOST HC_Texp_13 [aw=ch_pop]"

egen testo = total(ch_pop*(year==1963)), by(stfips)
replace ch_pop = testo
...
forval i = 3/3{		
	...
	*OUTREG THE ES RESULTS WITH TEST p-values AND DD ESTIMATE
	outreg2 using "$output/figureA2B3_add0.xls", `r' keep(HC_Texp_1 HC_Texp_3-HC_Texp_12) noparen noaster ctitle("X`i'") addstat(pre, `pre', post, `post', ddtest, `ddtest', pltest, `pltest', wlstest, `wlstest', dd, `dd', ddse, `ddse')
	local r append
}
