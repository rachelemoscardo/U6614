...
/*****************************GET DD STATISTICS********************************/
xmluse  "$output/figureA2C5_chrate.xls", clear  cells(A3:C65) first
...
graph set window fontface "Times New Roman"							/*FONT: make everything always show up as Times New Roman*/
	...
	twoway (scatter Nonwhite_1 ubnw lbnw nwdd  exp, 
			...
			text(-1.5 -7 "Nonwhite DD Estimate: `nwdd' (s.e. = `nwddse')" , j(left) size(small) box fcolor(white) margin(small))
			)
			(pcarrowi -1.5 -1.7 `nwdd' .8, lcolor(black) mcolor(black) lwidth(medthick) mlwidth(medthick))
			(pcarrowi 1 -3.8 .5 -1.1, lcolor(black) mcolor(black) lwidth(medthick) mlwidth(medthick)
			text(1 -7.5 "Year Before Medicaid", j(left) size(medium))
	...
	twoway (scatter White_1 ubw lbw  wdd exp, 
			lpattern(solid dash dash solid solid) 
			lcolor(midblue midblue midblue blue) 
			lwidth(thick medium medium medium medium thick thick)
			msymbol(Sh i i i i i i) 
			mcolor(midblue midblue midblue red blue) 
			c(l l l l l l l l l) 
			cmissing(n n n n n n n) 
			xline(-1, lcolor(black)) 
			yline(0, lcolor(black)) 
			...
			graphregion(fcolor(white) color(white) icolor(white) margin(zero)) 
			text(-3 -7 "White DD Estimate: `wdd' (s.e. = `wddse')" , j(left) size(small) box fcolor(white) margin(small))				
			)
			(pcarrowi -3 -2.1 `wdd' 4.8, lcolor(black) mcolor(black) lwidth(medthick) mlwidth(medthick))	
			(pcarrowi 4 -3.8 3 -1.1, lcolor(black) mcolor(black) lwidth(medthick) mlwidth(medthick)
			text(4 -7.5 "Year Before Medicaid", j(left) size(medium))
...
graph export "$output/figureA2C5_chrate.emf", replace
