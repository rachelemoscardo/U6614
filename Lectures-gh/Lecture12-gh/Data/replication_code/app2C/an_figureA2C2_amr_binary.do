...
*test DD assumption
	...
	local ddtest = r(p)
outreg2 using "$output/figureA2C2_amr_binary.xls", `r' keep(HC_Texp_2-HC_Texp_27) noparen noaster ctitle("Nonwhite") addstat(wlstest, `wlstest', ddtest, `ddtest', dd, `dd', ddse, `ddse', pre, `pre', post, `post')
local r append
...
*test DD assumption
	...
	test 	_b[HC_Texp_19] = _b[HC_Texp_20] =  ///
			...
			_b[HC_Texp_27], accumulate
	local ddtest = r(p)
outreg2 using "$output/figureA2C2_amr_binary.xls", `r' keep(HC_Texp_2-HC_Texp_27) noparen noaster ctitle("White") addstat(wlstest, `wlstest', ddtest, `ddtest', dd, `dd', ddse, `ddse', pre, `pre', post, `post')
local r append
