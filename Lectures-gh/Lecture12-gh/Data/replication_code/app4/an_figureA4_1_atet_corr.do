...
set more off
capture log close
log using "$output/pbs_atet_corr", replace text
program drop _all
...
program define pbs_corr
	...
	*but make it a BVN with covariance = `1'*se_itt*se_fs, which is equivalent to a correlation coeff of `1'
	matrix define V		= ($ittse^2, `1'*$ittse*$fsse \ `1'*$ittse*$fsse, $fsse^2)		
	drawnorm itt fs, cov(V) means($itt,$fs)					
	save "$datatemp/atet_`2'", replace
	...
	*3. Combine the normal draws for ITT and FS with the draws for the mortality adjustment
		merge 1:1 iter using "$datatemp/atet_`2'"
		drop _merge
	...
	*that I never have the extremes of the bootstrap draws as the top/bottom of the C.I.
	...
	gen uci = $uci 
	gen corr = `1'
	append using "$datatemp/pbs_atet_`2'_corr"
	save "$datatemp/pbs_atet_`2'_corr", replace
...
foreach type in /*cginf cgkids mwteens sbead sbewad sbenwad*/ gbnwnnmr gbnw14 gbnwkids{
	...
	*save a blank file to append the CIs to
	clear	
	save "$datatemp/pbs_atet_`type'_corr", replace emptyok
	*save a blank file to save the draws
	save "$datatemp/draws_`type'_corr", replace emptyok
		...
		*1. Read in spreadsheet of reduced form/ITT estimates, first stage estimates, and baseline mortality calculations
		...
		keep if type=="`type'"
		drop type
		*1. Step one was toget the parameters in and make globals out of them...done outside the loop
		*2. Define globals to loop through the bootstrap reps
		...
		/*********************************************************************/
		******DO THE BOOTSTRAP, LOOP THROUGH  CORRELATIONS B/W RF AND FS******
		/**********************************************************************/
		forval s = -1(.1)1{
			pbs_corr `s' `type'
		 }
