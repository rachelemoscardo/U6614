...
#6c. 
  ...
  #linear relationship (vary by nblack)
    ggplot(stations_wcrime, aes(x = crimes, y = arrperswipe, color = nblack)) +
      geom_point()  +
      geom_smooth(method = 'lm', formula = y ~ x) + 
      ylab("Arrests per 100,000 ridership") + xlab("Station area poverty rate") +
      ggtitle("Fare Evasion Arrest Intensity vs Poverty by Race", 
              subtitle = "Subway stations in Brooklyn (2016)") +
      scale_color_discrete(name = "Predominantly Black Station",
                           labels=c("No", "Yes"),
  ...
  #compare adj r-square of full linear models (w/crime and poverty) estimated separately by race
    stations_wcrimes_black <- stations_wcrime %>% filter(nblack == "Majority Black")
    stations_wcrimes_nonblack <- stations_wcrime %>% filter(nblack == "Majority non-Black")
    
    olscl_black <- lm(arrperswipe ~ crimes,
                      data = stations_wcrimes_black) #include quadratic term
    summary(olscl_black) 
    
    olscl_nonblack <- lm(arrperswipe ~ crimes,
                         data = stations_wcrimes_nonblack) #include quadratic term
    summary(olscl_nonblack) 
    
    
  #compare adj r-square of full linear models (w/crime and poverty) estimated separately by race
    olsfl_black <- lm(arrperswipe ~ povrt_all_2016 + crimes,
                data = stations_wcrimes_black) #include quadratic term
    summary(olsfl_black) 
    
    olsfl_nonblack <- lm(arrperswipe ~ povrt_all_2016 + crimes,
                      data = stations_wcrimes_nonblack) #include quadratic term
    summary(olsfl_nonblack) 
