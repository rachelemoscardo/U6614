...
##    - only keep new cols for each analysis var, `Country Name`, `Country Code`
  ...
  stem.fmshstemgrads <- stem.fmshstemgrads %>% 
    ...
    summarise(fmshstemgrads = max(female_share, na.rm=TRUE))
  ...
  stem.dayspaidmatleave  <- stem.dayspaidmatleave  %>% 
    pivot_longer(cols = `2011 [YR2011]`:`2020 [YR2020]`,
                 ...
                 values_to = "female_share") %>% 
    group_by(`Country Name`,`Country Code`) %>% 
    summarise(dayspaidmatleave = max(female_share, na.rm=TRUE))
  ...
  stem.unmetcontr <- stem.unmetcontr %>% 
    pivot_longer(cols = `2011 [YR2011]`:`2020 [YR2020]`,
                 ...
                 values_to = "female_share") %>% 
    group_by(`Country Name`,`Country Code`) %>% 
    summarise(unmetcontr = max(female_share, na.rm=TRUE))
...
##    - including 3 analysis variables and `Country Name` and `Country Code`
  ...
  stem.cross <- stem.fmshstemgrads %>% 
    inner_join(stem.unmetcontr) %>% 
