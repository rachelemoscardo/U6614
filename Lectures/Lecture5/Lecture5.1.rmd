...
3. Obtain the sample size and dimensions of the dataset.
...
* Our ultimate analysis sample collapsed the microdata to the subway station level, i.e. the *unit of analysis* was the subway station area.

Note that we chose to collapse the data to aggregate units because it allowed us to **exploit variation that was more useful** for exploring racial disparities in enforcement. Instead of exploiting variation in race across arrested individuals, we exploited variation in racial composition across neighborhoods.
...
1. Never modify an original variable. 
    + Instead, create new variables based on original variables. You can deselect the old ones when you're ready, but you want to keep the original variables so you can validate your new variables against them.
...
We can visualize the distribution of a continuous variable with a histogram using `ggplot()` and the `geom_histogram()` geometry, see Lecture 3.1.9. We can also compute the mean, median, standard deviation, min, max, and percentiles. We reviewed the `summary()`, `summarize()` and `count()` functions for computing summary statistics in Lecture 3.1.7. If quantiles or percentiles are of interest, try the `quantiles()` function. 

Here are some examples of R code used to analyze arrests per 100k swipes (a continuous variable) in our grouped subway-station level data from last week.
...
#note NAs recognized by R will prevent calcs unless na.rm = TRUE
summary(stations$arrperswipe) 
...
Recall from Lecture 3 that the `ifelse()` function is a handy function to use within `mutate()` to recoding values from an input variable and create a new clean variable.  `recode()` is another handy function that respects factor labeling.

Recoding and transforming is often part of an iterative process, where we explore and describe data, manipulate the data, then describe again through summary statistics or charts. It's important to continuously **validate** the data along the way, lest we end up working with data that doesn't make sense.
...
ln_income <- ln(income) 
  #ln() is in the SciViews package and is equivalent to log() in base R
```
...
#4. compute P-value column
  ...
  #get p-values for coef of interest in each model
...
Last week in class we used the base R function `weighted.mean()`, and the `wtd.t.test()` function from the `weights` package. The weights package provides some additional functions that utilize weights, including `wtd.cor()` and `wpct()`. 
...
The `ggplot` package accepts weights as an argument to the aesthetic mapping function `aes()`:
```{r, message = FALSE, warning = FALSE}
