...
#subtitle: "Importing, Exporting, Appending, Cleaning, and Summarizing Data"
...
The `utils` package that is automatically loaded with R includes a set of functions for importing data (e.g. `read.csv()`). Comparable functions included in the `readr` package (e.g. `read_csv()`) are generally faster and import data as tibbles that work well `tidyverse` packages. We'll focus on the `readr` functions for importing data, which are included as part of the [Tidyverse](https://www.tidyverse.org/packages/) suite of packages.


## Comma delimited files (.csv) 

Many datasets are stored at text files with rows of data and values for each column separated by commas (or some other *delimiter*).

```{r eval = FALSE}
df1 <- read_csv("yourfilename.csv")
```

The default is for strings to be stored as characters rather than factors (unlike `read.csv()` in the `utils` package). One common option you can set is to specify which values are read in as `NA`, in this example we'll treat blank character values as NAs. Another option allows you to specify column names.
```{r eval = FALSE}
...
R also has its own data file format, .rds, that you can easily read in using the `readRDS()` function in base R.
```{r eval = FALSE}
df3 <- readRDS("yourrdatafile.rds")
```
...
Here is the generic syntax for appending using `rbind()`.
```{r eval = FALSE}
...
`recode()` is another dplyr function for replacing numeric, character or factor values. Say we want to recode gapminder observations listed with the country name "West Bank and Gaza" to instead refer to the country name "Occupied Palestinian Territories" (the name recognized by the UN, EU and more). We can use the `recode()` function as part of the argument in the `mutate()` function to create a new variable called `country_clean`.
```{r}
g2 <- gapminder %>% 
...
```
Note that `country` is factor, so categorical values (levels) are stored as integers but displayed as characters. We can refer to the levels themselves when using `recode()`.
...
The `forcats` package included in the tidyverse includes a number of tools for working with factors. `forcats` includes a set of functions for re-ordering factor levels, which can be useful when we summarizing factor variables.
```{r message = FALSE, warning = FALSE}
...
gapminder$continent %>% 
  ...
  levels()
```
Take a look at the forcats [documentation](https://forcats.tidyverse.org/) for a cheat sheet that describes other functions for working with factor variables.


## if_else

`if_else` is a useful dplyr function for manipulating data in (non-factor) columns of data based on conditional statements. To get the gist of how `if_else` works, let's create a binary (logical) variable called `cont_asia` equal to `TRUE` if `continent == "Asia"` and `FALSE` for other continents.
```{r message = FALSE, warning = FALSE}
g3 <- gapminder %>% 
...
A more flexible function for computing summary statistics is the dplyr's `summarize()` function. `summarize()` stores results as data frames and can be used to perform more complicated calculations in comibination with `group_by()`. This can be helpful for characterizing *conditional distributions*, such as computing statistics for one variable conditional on the value of another variable. (Crosstabs, discussed in the next section, are another tool for showing *conditional* or *joint* distributions.)
...
The base R `table()` function is a quick tool for generating frequency tables (also known as one-way tables) and crosstabs (two-way tables). Note that passing the data column `continent` as an argument to the `table()` function yields equivalent results as passing it to the `summary` function.
```{r}
table(g3$continent)
```
...
- **aesthetics** map variables to visual properties like color, shape, size, etc. **Aesthetic mappings are set as arguments to the `aes()` function, which must be specified along with a `ggplot()` call. ggplot2 knows to look for mapped variables in side the data argument
...
Here is the same information with additional options to display cointent proportions instead of counts.
```{r message = FALSE, warning = FALSE}
# but R is smart enough to recognize many arguments without specifying by name
