...
R also has its own data file format, .rds, that you can use to save objects (data frames as well as other R objects) and easily read in using the base R `readRDS()` function.
```{r eval = FALSE}
...
`recode()` is another dplyr function for replacing numeric, character or factor values. Say we want to recode gapminder observations listed with the country name "West Bank and Gaza" to instead refer to the country name "Occupied Palestinian Territories" (the name recognized by the UN, EU and more). We can use the `recode()` function as part of the argument for the `mutate()` function in order to create a new variable called `country_clean`.
```{r}
...
```
Note that `country` column is a factor, so categorical values (levels) are stored as integers but displayed as characters. We can refer to the levels themselves when using `recode()`.

`recode()` can also be used to recode values in numeric, character, and logical columns of data.

`replace()` is a base R function that is can also be used for recoding values.
...
#assign the contents of this vector to the levels attribute of factor var1
df %>% mutate(var1.fac = factor(var1, levels = levels_to_keep))
```
...
A more flexible function for computing summary statistics is the dplyr `summarize()` function. `summarize()` stores results as data frames (as tibbles, as is the norm in the Tidyverse) and can be used to perform more complicated calculations in combination with `group_by()`. 

This can be helpful for characterizing *conditional distributions*, such as computing statistics for one variable conditional on the value of another variable. (Crosstabs, discussed in the next section, are another tool for showing *conditional* or *joint* distributions.)
...
Here is the code to visualize the distribution of life expectancy across countries in 2007 using a very basic histogram with no additional options specified.
```{r message = FALSE, warning = FALSE}
