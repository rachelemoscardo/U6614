...
#3c. race_eth
  ...
  #let's investigate a bit
    ...
    prop.table(table(arrests_bds.clean$race_clean, 
                     arrests_bds.clean$hispanic), 2) %>% round(2)
...
#4b. validate
  ...
  table(arrests_las.clean$race_eth, arrests_las.clean$las_race_key, 
        useNA = "always")
  table(arrests_las.clean$race_eth, arrests_las.clean$hispanic_flag, 
        useNA = "always")
...
#remember to use na.rm = TRUE when calculating means
  race_eth_stats <- FILL IN CODE HERE 
...
#7c. 
  arrests_stations_top <- FILL IN CODE HERE
...
#8c.
  ...
  saveRDS(arrests_stations_race_top, "./arrests_stations_race_top.rds")
