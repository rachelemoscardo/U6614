...
Race and ethnicity are two aspects of personal identity that we'll be exploring with data throughout this class. This document is a collection of observations based on short articles that can help us think about what to make of the data we encounter, and how to process data to better explore the connections between race, ethnicity and policy.

Richer and more nuanced data on personal identity can help us better understand the ways that systems of oppression interact with identity to limit opportunity. Data can improve transparency and [disrupt algorithmic bias](https://www.jstor.org/stable/10.5325/jinfopoli.8.2018.0078#metadata_info_tab_contents). But how do we get the most of the limited data we have access to now?
...
The takeaway for policy analysis is not to take the pre-defined categories at face value, but to think about recoding race and ethnicity data into identity categories that are most meaningful given the policy context under study.

![](censusracequestion.jpg){width=40%}

# Multi-dimensional identity and intersectionality

Simplicity can be a good thing when it comes to data accessibility, but reducing people to a single dimension of identity is obviously limiting in many ways. Modeling how intersectional identity relates to outcomes should allow for [multiplicative interactions](https://weallcount.com/2019/06/21/intersectionality-data-2/) between dimensions of identity rather than additively separable components of identity. 

What if the policy context focuses on measures that reduce identity to a single dimension? For example, if native English fluency is particularly relevant for a given research question, that might suggest preserving and centering measures of Hispanic identity in the data. If policing and colourism is the focal point, perhaps data coding should give precedence to Black racial identity over Hispanic ethnicity? 

Or even better, maybe we can restructure the data to measure the prevalence of a certain dimension of identity within broader groups (e.g. neighborhoods or cohorts) rather than prescribing limited measures of identity to individuals. This is something we'll come back when we study NYPD enforcement in the coming weeks.
...
I'd say the answer is an unsatisfying... maybe. Would your decision to relabel Hispanic identity as Latinx substantially mispresent the underlying data, or enrich the meaning of identity on balance? How will the identity labels affect how different audiences engage with your analysis?
