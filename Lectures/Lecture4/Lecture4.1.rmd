...
nlevels(gap_joined_inner$country)
```
Here we can see all 142 countries are retained from the gapminder data, even though we know that 12 countries from the gapminder data were not matched to countries in the health expenditure data.
...
gap_global <- gapminder %>% 
  ...
  summarize(population = sum(pop), gdp = sum(gdp_tot)) %>% # compute global pop and GDP
