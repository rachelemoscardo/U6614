...
OK, so R refers to this type of object as `language` with `class = formula`. What can we do with this formula? Lot of thing! Like specify a population regression function, for one:
```{r}
...
But wait a minute, **what formula is R using to estimate the standard error (SE) of the difference??** 
...
Notice how the SE of the difference in means changes slightly when the SE formulas allow for unequal variances:
```{r, message = FALSE, warning = FALSE}
...
```
Notice we get the same t-statistic as above, 4.6468!
...
```
Now we have the same t-statistic (4.64) as with the conventional difference-in-means t-test assuming unequal population variances.
