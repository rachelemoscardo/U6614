...
But wait a minute, what formula is R using to estimate the standard error of the difference?? We probably want to allow for unequal variances in the different group populations, which is equivalent to robust SEs in the analagous bivariate regression setting (we'll get to the bivariate regression framing shortly!). Here the default option is to assume the sample means are drawn from populations with unequal variances, but can we still explicitly specify the variance formula as an additional option. Consult Stock and Watson Chapter 3.4 (3rd edition) for a refresher on difference-in-mean tests and the appropriate SE formula.
```{r, message = FALSE, warning = FALSE}
