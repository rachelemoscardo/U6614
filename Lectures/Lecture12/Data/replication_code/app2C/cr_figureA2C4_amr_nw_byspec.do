...
/*****************************GET DD STATISTICS********************************/
xmluse  "$output/figureA2C4_amr_nw_byspec.xls", clear  cells(A3:H54) first
...
twoway (scatter X3_1 X1_1 X5_1 lb ub exp, 
		...
		xsize(7.5) ysize(5.5) 			
		legend(order(1 2 3) rows(3) label(1 "Preferred (Col. 2)") label(2 "Simple (Col. 1)") label(3 "Linear Trends (Col. 4)") ring(0) bplace(sw) bmargin(small) rowgap(.1) size(small))
...
twoway (scatter X3_1 X6_1 lb ub exp, 
		...
		ylabel(, labsize(medium))
		xsize(7.5) ysize(5.5) 			
		legend(order(1 2) rows(2) label(1 "Preferred (Col. 2)") label(2 "DDD (Col. 5)") ring(0) bplace(sw) bmargin(small) rowgap(.1) size(small))
		xtitle("Years since Medicaid Implementation", size(medsmall))
		ytitle("log Mortality Rate x 100", size(medsmall))
...
twoway (scatter X3_1 X7_1 lb ub exp, 
		...
		ylabel(, labsize(medium))
		xsize(7.5) ysize(5.5) 			
		legend(order(1 2) rows(2) label(1 "Preferred (Col. 2)") label(2 "1958 IV (Col. 6)") ring(0) bplace(sw) bmargin(small) rowgap(.1) size(small))
		xtitle("Years since Medicaid Implementation", size(medsmall))
		ytitle("log Mortality Rate x 100", size(medsmall))
...
graph export "$output/figureA2C4_amr_byspec_nonwhite.emf", replace
