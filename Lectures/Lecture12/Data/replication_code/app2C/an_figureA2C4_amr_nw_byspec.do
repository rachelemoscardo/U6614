*create the 1958 ADC Instrument for the IV model
...
forval i = 1/5{
	...
	reg lnamrch `DX`i'' if ~white, cluster(stfips)
	local dd = _b[HCPOST]
	local ddse = _se[HCPOST]
	*Run model `i'
	reg lnamrch `X`i'' if ~white, cluster(stfips)
	outreg2 using "$output/figureA2C4_amr_nw_byspec.xls", `r' keep(HC_Texp_2-HC_Texp_27) noparen noaster ctitle("X`i'") addstat(dd, `dd', ddse, `ddse')
	local r append
	if `i'==4{
		restore
	}	
...
for var HC*: replace X = X*(white==0)
...
outreg2 using "$output/figureA2C4_amr_nw_byspec.xls", `r' keep(HC_Texp_2-HC_Texp_27) noparen noaster ctitle("X6") addstat(dd, `dd', ddse, `ddse')
...
outreg2 using "$output/figureA2C4_amr_nw_byspec.xls", `r' keep(HC_Texp_2-HC_Texp_27) noparen noaster ctitle("X7") addstat(dd, `dd', ddse, `ddse')
