...
/*****************************GET DD STATISTICS********************************/
xmluse  "$output/figureA2C2_amr_binary.xls", clear  cells(A3:C65) first
...
for var ubw lbw: replace X = . 
	#delimit ;
	twoway (scatter White_1 Nonwhite_1 lbw ubw ubnw lbnw nwdd wdd exp, 
			...
			text(-.1 -7 "Nonwhite DD Estimate: `nwdd' (s.e. = `nwddse')" , j(left) size(small) box fcolor(white) margin(small))
			text(.1 4.2 "White DD Estimate: `wdd' (s.e. = `wddse')" , j(left) size(small) box fcolor(white) margin(small))			
			)
			(pcarrowi -.1 -1.3 `nwdd' .8, lcolor(black) mcolor(black) lwidth(medthick) mlwidth(medthick))
			(pcarrowi .1 4.8 `wdd' 4.8, lcolor(black) mcolor(black) lwidth(medthick) mlwidth(medthick))
			(pcarrowi .08 -3.8 .06 -1.1, lcolor(black) mcolor(black) lwidth(medthick) mlwidth(medthick)
			text(.085 -7.1 "Year Before Medicaid", j(left) size(medium)));
			;
	#delimit cr;	
	
graph display, 	xsize(7.5) ysize(5.5) 
graph export "$output/figureA2C2_amr_binary.emf", replace
