...
char exp[omit] -1
xi i.exp, pref(_T)
for var _Texp*: gen HCX = chrate0*X
...
forval i = 3/3{		
	...
	*OUTREG THE ES RESULTS WITH TEST p-values AND DD ESTIMATE
	outreg2 using "$output/figureA2B4_fs_chrate.xls", `r' keep(HC_Texp_2-HC_Texp_11) noparen noaster ctitle("X`i'") addstat(pre, `pre', post, `post', ddtest, `ddtest', pltest, `pltest', wlstest, `wlstest', dd, `dd', ddse, `ddse')
	local r append
}
