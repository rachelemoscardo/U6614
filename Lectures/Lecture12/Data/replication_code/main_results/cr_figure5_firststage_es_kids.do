...
xmluse  "$output/figure5.xls", clear  cells(A3:B34) first
drop in 1
...
foreach stat in pre post ddtest pltest wlstest {
	...
	local `stat' : display %03.2f round(r(mean)*100)/100
}
...
graph export "$output/figure5_firststage_kids.emf", replace
