...
Here we choose not to weight station observations by ridership, so that each station area is equally weighted in the regression analysis. Here weighting by station ridership would give less weight when estimating the regression to less busy stations in more peripherally located neighborhoods of color where they may be heightened enforcement. Weighting by ridership also exacerbates a measurement error problem that we will discuss further in class. When computing statistics for groups of stations in the next section (e.g. stations in high-poverty neighborhoods), we do weight by ridership so that statistics are representative of ridership in each group.

If you prefer the linear specification because you find it simpler to interpret without changing the substantive conclusions, that is a reasonable justification.
...
ggplot(stations, aes(x = povrt_all_2016, y = arrperswipe, color = nblack)) +
    ...
    ylab("arrests per 100,000 ridership") + xlab("poverty rate") +
